...
const handleProductCreate = async (values, { resetForm, setFieldValue }) => {
  ...
  useEffect(() => {
    if (editProduct?.productImage) {
      setFile(
        `${process.env.NEXT_PUBLIC_BASE_URL}/${editProduct.productImage}`
      );
    }
  }, [editProduct]);
                ...
                {typeof values.datasheetFile === "string" &&
                  values.datasheetFile && (
                    <div className="mt-2 text-sm">
                      ...
                      <a
                        href={`${process.env.NEXT_PUBLIC_BASE_URL + values.datasheetFile}`}
                        target="_blank"
                ...
                {typeof values.connectionDiagramFile === "string" &&
                  values.connectionDiagramFile && (
                    <div className="mt-2 text-sm">
                      <span className="text-gray-700">Current file: </span>
                      <a
                        href={`${process.env.NEXT_PUBLIC_BASE_URL + values.connectionDiagramFile}`}
                        target="_blank"
                        className="text-blue-600 underline"
                      >
                        {getOriginalFilename(
                ...
                {typeof values.userManualFile === "string" &&
                  values.userManualFile && (
                    <div className="mt-2 text-sm">
                      <span className="text-gray-700">Current file: </span>
                      <a
                        href={`${process.env.NEXT_PUBLIC_BASE_URL + values.userManualFile}`}
                        target="_blank"
                        className="text-blue-600 underline"
                      >
                        {getOriginalFilename(
